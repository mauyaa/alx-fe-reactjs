...
export default function TodoList() {
  ...
  const addTodo = (text) => setTodos((p) => [{ id: nextId++, text, completed: false }, ...p]);
      ...
      <ul aria-label="todo-list">
        {todos.map((t) => (
          <li key={t.id}>
            ...
            <button type="button" onClick={() => deleteTodo(t.id)} aria-label={`Delete ${t.text}`} style={{ marginLeft: '.75rem' }}>
              Delete
            </button>
          </li>
        ))}
